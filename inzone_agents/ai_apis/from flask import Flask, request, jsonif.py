...
def create_popular_character():
    try:
        ...
        # ---------- choose voice_id ----------
        voice_id, selected_title, voice_reason = choose_voice_id(
            name=name,
            personality=personality,
            greeting=greeting
        )
        ...
        character_ref.set({
            ...
            "numberOfChats"       : number_of_chats,
            "personality"         : personality,
            "profile_picture_url" : public_urls[0],
            "votes"               : votes,
            "created_by_human"    : created_by_human,
        ...
        app.logger.info(f"[voice] {character_ref.id} title={selected_title} reason={voice_reason} -> {voice_id}")

        return jsonify({
            "success"             : True,
            "PopularCharacterId"  : character_ref.id,
            "profile_picture_url" : public_urls[0],
            "candidate_images"    : public_urls,
            "voice_id"            : voice_id,
            "voice_title"         : selected_title,
...
voices_map = {
    "Neutral Female, Early 20s – American General": "bR7lTtN7R9lnwR25LPQc",
    "Neutral Male, Mid-20s – American General": "gSElG2eceXY0VUzqQ6b4",
    "Neutral Androgynous, Teen/Young Adult – American General": "FGpa86bZRP2baUflOy5r",
    "Neutral Female, 30s – American General": "gpyQQyuuMX6lRlJaHebu",
    "Neutral Male, 30s – American General": "Jz3oxjjyKhXEGxcGgF5K",
    "Neutral Female, Late 40s – American General": "sdhi8K18LMYf2cvxeBVFTaf",
    "Neutral Male, Late 40s – American General": "uAsdnJu3fC98UFujT7cf",
    "Neutral Androgynous, Youthful – American General": "dsFLBQnSxe2xNzbxZh6S",
    "Male, Mid-20s – British RP (Received Pronunciation)": "yV3edNyKvFOYOZ5UuzGq",
    "Female, 20s – British RP": "Tmz6gqPr0SambtZyjgbo",
    "Male, 30s – British Cockney": "TZXtHkFTCwgE3KtwrCEL",
    "Female, 20s – Scottish Accent": "0ghI6PDYc5ld88HDFzGe",
    "Male, 20s – Australian Accent": "nknYJQ5xHqW3NCl1UfOr",
    "Female, 20s – African English (Nigerian)": "UC0k3KP6JY4PHFaVxqOE",
    "Male, 30s – Indian English": "JFXkk7z48Q50P9g1ttZu",
    "Female, 30s – Caribbean English": "ZY0uH29YJkrDpTk2ZmsL",
    "Male, 30s – Raspy/Gravelly American": "1Ha5DpZmJxruUO9FPiIK",
    "Female, 20s – Raspy/Smoky American": "SKNS8zgVgB8xZtb1feXP",
    "Male, 20s – High Energy, Comedic": "0sNrFUxgBWKh4PJmraAD",
    "Female, 20s – High Energy, Playful": "SMHkF9u8z4tpQZiNqJ14",
    "Male, 40s – Booming/Commanding": "m505z0IoHQX376wch1Je",
    "Female, 40s – Authoritative, Sharp": "gAqMXbeBidEgT0XUGDZ7",
    "Androgynous, Soft/Whispery": "w8MuJ0ix1VcNrwYsPtFO",
    "Male, 20s – Warm, Boy-Next-Door": "nwUVKzsYAwthxc6Yyhaw",
    "Elderly Male, 60s+": "0GJ4gpdKlKSLZQpiwKnI",
    "Elderly Female, 60s+": "5KcYBUokU14cL9izkCL1",
    "Teen Male, 15–17": "cOR74nR9Jm8qG2LIT6IR",
    "Teen Female, 15–17": "WwCMdutA5X2FMmtkDGst",
}

def _build_voice_messages(name: str, personality: str, greeting: str, titles: list):
    return [
        {
            "role": "system",
            "content": (
                "You are a strict voice curator. Choose exactly one title from the provided list. "
...
def choose_voice_id(name: str, personality: str, greeting: str):
    titles = list(voices_map.keys())
    messages = _build_voice_messages(name or "", personality or "", greeting or "", titles)
    ...
    try:
        ...
        voice_id = voices_map.get(title)

        if voice_id:
            return voice_id, title, reason

        fallback_title = "Neutral Male, 30s – American General"
        return voices_map[fallback_title], fallback_title, "fallback mapping because title not in map"

    except Exception as e:
        fallback_title = "Neutral Male, 30s – American General"
        return voices_map[fallback_title], fallback_title, f"fallback due to error: {e}"
